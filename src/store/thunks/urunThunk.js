import {setCategories, setBestSellers, setFetchState, setTotal, setProductDetail, setProductList} from "../actions/urunAction";
...
export const getProducts = ({limit, offset}) => async (dispatch, getState) => {
    ...
    try {
        ...
        const response = await axiosOrnek.get(`/products?limit=${limit}&offset=${offset}`);
...
export const getProductsByCategory = ({categoryId, limit, offset}) => async (dispatch) => {
    try {
        ...
        const response = await axiosOrnek.get(`/products?category=${id}&limit=${limit}&offset=${offset}`);
        const {products, total} = response.data;
...
export const getFilteredProducts = ({categoryId, filter, sort, limit, offset}) => async (dispatch) => {
    try {
        ...
        const request = [categoryId && `category=${categoryId}`,
            ...
            `limit=${limit}`,
            `offset=${offset}`].join("&");
        const response = await axiosOrnek.get(`/products?${request}`);
        const {products, total} = response.data;
...
export const getProductDetail = (productId) => async (dispatch) => {
    try {
        dispatch(setFetchState("FETCHING"));
        const response = await axiosOrnek.get(`/products/${productId}`);
